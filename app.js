...
(() => {
  ...
  function renderHome() {
    ...
    const categoriesHtml = state.categories.length
      ? state.categories.map((c) => {
          ...
          return `<button class="notebook-category-card" data-home-category="${c.id}">
      ...
      : `<div class="notebook-empty">Your notebook is waiting for its first category.</div>`;
  ...
  function renderCategoryDetail(catId) {
    ...
    const toolbar = `<button class="btn btn-primary btn-sm category-detail-add" id="category-detail-add">＋ Add</button>`;
  ...
  async function handleFormSubmit(e, type, existingItem, tagCtl, isQuick) {
    ...
    if (type === "note") {
      ...
      item.categoryId = state.activeCategoryId || (state.categories[0] ? state.categories[0].id : undefined);
