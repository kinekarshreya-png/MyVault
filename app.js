...
(() => {
  const state = {
    ...
    trashItems: [],
    view: "home",
    sublistContext: null, // { kind: 'category'|'important'|'reminders'|'trash', catId? }
    taskTab: "today",
    searchQuery: "",
    searchFilter: "all",
    ...
    activeCategoryId: null, // category currently open / receiving new items
  };

  // Start with a clean notebook. Categories are created by the user from the dashboard.
  ...
  function attachBlobUrls(items) {
    for (const item of items) {
      if (item.type === "image") {
        const blobs = Array.isArray(item.imageBlobs) ? item.imageBlobs : (item.fileBlob instanceof Blob ? [item.fileBlob] : []);
        item._imageUrls = blobs.map((blob) => {
          const url = URL.createObjectURL(blob);
          state.blobUrls.push(url);
          return url;
        });
        if (item._imageUrls.length) item._blobUrl = item._imageUrls[0];
      } else if (item.fileBlob instanceof Blob && item.type === "video") {
  ...
  function wireGlobalEvents() {
    ...
    // Dashboard quick-add buttons and category cards
    document.getElementById("home-sections").addEventListener("click", (e) => {
      const overdueAction = e.target.closest("[data-overdue-action]");
      if (overdueAction) {
        e.stopPropagation();
        handleOverdueAction(overdueAction.dataset.overdueAction, overdueAction.dataset.id);
        return;
      }
      const addBtn = e.target.closest("[data-home-add]");
      if (addBtn) {
        state.activeCategoryId = null;
        openItemForm(addBtn.dataset.homeAdd);
        return;
      }
      const catAdd = e.target.closest("#home-add-category");
      if (catAdd) {
        openCategoryForm();
        return;
      }
      ...
      if (catAction) {
        const cat = state.categories.find((c) => c.id === catAction.dataset.categoryId);
        if (!cat) return;
        if (catAction.dataset.categoryAction === "edit") openCategoryForm(cat);
        if (catAction.dataset.categoryAction === "delete") deleteCategoryWithItems(cat);
        return;
      }
      const cat = e.target.closest("[data-home-category]");
      if (cat) {
        renderCategoryDetail(cat.dataset.homeCategory);
      }
    });
    ...
    document.getElementById("category-cancel-btn").addEventListener("click", () => closeSheet("category-sheet-overlay"));
    ...
    // Sublist back
    document.getElementById("sublist-back").addEventListener("click", () => {
      if (state.sublistContext && state.sublistContext.kind === "category") {
        state.activeCategoryId = null;
        state.sublistContext = null;
        goToView("home");
      } else {
        goToView("more");
      }
    ...
    document.getElementById("tasks-list").addEventListener("click", (e) => {
      const overdueAction = e.target.closest("[data-overdue-action]");
      if (overdueAction) { handleOverdueAction(overdueAction.dataset.overdueAction, overdueAction.dataset.id); return; }
  ...
  function renderHome() {
    ...
    const todayTasks = state.items
      .filter((i) => i.type === "task" && !i.completed && (!i.dueDate || (i.dueDate >= todayStartMs && i.dueDate <= todayEndMs)))
      .sort((a, b) => (a.dueDate || Infinity) - (b.dueDate || Infinity));
    const reminders = state.items
      .filter((i) => i.reminderAt && !i.completed)
      .sort((a, b) => a.reminderAt - b.reminderAt);

    const overdueTasks = state.items
      .filter((i) => i.type === "task" && !i.completed && i.dueDate && i.dueDate < todayStartMs)
    ...
    const taskHtml = todayTasks.length
      ? todayTasks.map(UI.taskRow).join("")
      : UI.emptyState("No tasks for today yet.");
    const overdueHtml = overdueTasks.length
      ? `<div class="overdue-block">
          <div class="overdue-heading">⚠ ${overdueTasks.length === 1 ? "Yesterday's unfinished task" : "Unfinished tasks from earlier"}</div>
          ${overdueTasks.map((task) => UI.taskRow(task, { overdueActions: true })).join("")}
        </div>`
      : "";
    const reminderHtml = reminders.length
      ? reminders.map(UI.reminderRow).join("")
      : UI.emptyState("No reminders yet.");

    const categoriesHtml = state.categories.map((c) => {
      const count = state.items.filter((i) => i.categoryId === c.id).length;
      return `<div class="notebook-category-card" data-home-category="${c.id}">
        <span class="category-tape" style="background:${c.color}"></span>
        <div class="category-card-main">
          <span class="category-card-name">${Utils.escapeHtml(c.name)}</span>
          <span class="category-card-count">${count} ${count === 1 ? "item" : "items"}</span>
        </div>
        <div class="category-card-actions">
          <button type="button" class="category-mini-btn" data-category-action="edit" data-category-id="${c.id}" aria-label="Edit ${Utils.escapeHtml(c.name)}">✎</button>
          <button type="button" class="category-mini-btn category-mini-danger" data-category-action="delete" data-category-id="${c.id}" aria-label="Delete ${Utils.escapeHtml(c.name)}">×</button>
    ...
    container.innerHTML = `
      <section class="notebook-section today-paper">
        <div class="notebook-section-head">
          <div><div class="paper-kicker">TODAY</div><h2>Things to do</h2><div class="paper-date">${todayDateLabel}</div></div>
          <button class="paper-add" data-home-add="task" aria-label="Add today's task">＋</button>
        </div>
        ${overdueHtml}
        <div class="paper-lines">${taskHtml}</div>
      </section>

      <section class="notebook-section reminder-paper">
        <div class="notebook-section-head">
          <div><div class="paper-kicker">DON'T FORGET</div><h2>Reminders</h2></div>
          <button class="paper-add" data-home-add="reminder" aria-label="Add reminder">＋</button>
        </div>
        <div>${reminderHtml}</div>
      </section>

      <div class="notebook-divider"><span></span><span></span></div>

      <section class="categories-notebook">
        <div class="category-heading">
          <div><div class="paper-kicker">MY NOTEBOOK</div><h2>Categories</h2></div>
        </div>
        <div class="category-grid">${categoriesHtml}</div>
        <button class="big-category-add" id="home-add-category">＋ Add a new category</button>
      </section>`;
  }
  ...
  async function handleOverdueAction(action, id) {
    const item = await MyVaultDB.getItem(id);
    if (!item || item.type !== "task" || item.completed) return;
    if (action === "today") {
      const d = new Date();
      d.setHours(23, 59, 59, 999);
      item.dueDate = d.getTime();
      item.updatedAt = Utils.now();
      await MyVaultDB.putItem(item);
      await refreshData();
      Utils.toast("Task moved to today.");
      rerenderCurrentView();
    } else if (action === "cancel") {
      await softDeleteItem(id);
      Utils.toast("Task cancelled and moved to Trash.");
      rerenderCurrentView();
    }
  }
  ...
  function renderCategoriesSublist() {
    ...
    const body = state.categories.map((c) => {
      ...
      return `<div class="category-tile" data-cat-id="${c.id}">
        ...
        <div class="category-tile-main"><span>${Utils.escapeHtml(c.name)}</span><span class="cat-count">${count} item${count === 1 ? "" : "s"}</span></div>
        <div class="category-tile-actions">
          <button type="button" class="category-mini-btn" data-category-action="edit" data-category-id="${c.id}">✎</button>
          <button type="button" class="category-mini-btn category-mini-danger" data-category-action="delete" data-category-id="${c.id}">×</button>
        </div>
      </div>`;
    ...
    document.getElementById("sublist-content").onclick = (e) => {
      const action = e.target.closest("[data-category-action]");
      if (!action) return;
      e.stopPropagation();
      const cat = state.categories.find((c) => c.id === action.dataset.categoryId);
      if (!cat) return;
      if (action.dataset.categoryAction === "edit") openCategoryForm(cat);
      if (action.dataset.categoryAction === "delete") deleteCategoryWithItems(cat);
    };
  ...
  function renderCategoryDetail(catId) {
    ...
    state.sublistContext = { kind: "category", catId: cat.id };
    const items = state.items.filter((i) => i.categoryId === cat.id).sort((a, b) => b.updatedAt - a.updatedAt);
    const body = items.length
      ? items.map((i) => i.type === "myapp" ? UI.myAppCard(i, state.categories) : UI.itemCard(i, state.categories)).join("")
      : UI.emptyState("This category is empty. Tap ＋ Add to save your first thing here.");
    const toolbar = `<div class="category-detail-toolbar"><button class="btn btn-primary btn-sm category-detail-add" id="category-detail-add">＋ Add</button><button class="btn btn-ghost btn-sm" id="category-detail-edit">Edit</button><button class="btn btn-danger btn-sm" id="category-detail-delete">Delete</button></div>`;
    showSublist(cat.name, toolbar, body);
    document.getElementById("category-detail-add").onclick = () => {
      state.activeCategoryId = cat.id;
      openAddSheet();
    };
    document.getElementById("category-detail-edit").onclick = () => openCategoryForm(cat);
    document.getElementById("category-detail-delete").onclick = () => deleteCategoryWithItems(cat);
  }
  ...
  // CATEGORY FORM
  // ---------------------------------------------------------------
  function openCategoryForm(existingCategory = null) {
    const title = document.getElementById("category-sheet-title");
    const submit = document.getElementById("category-submit-btn");
    const nameInput = document.getElementById("cat-name");
    title.textContent = existingCategory ? "Edit Category" : "New Category";
    submit.textContent = existingCategory ? "Save Changes" : "Create Category";
    nameInput.value = existingCategory ? existingCategory.name : "";

    const row = document.getElementById("cat-color-row");
    let selected = existingCategory?.color || SWATCHES[0];
    ...
    document.getElementById("category-form").onsubmit = async (e) => {
      ...
      const cat = existingCategory
        ? { ...existingCategory, name, color: selected, updatedAt: Utils.now() }
        : { id: Utils.uid(), name, color: selected, createdAt: Utils.now() };
      await MyVaultDB.putCategory(cat);
      closeSheet("category-sheet-overlay");
      await refreshData();
      Utils.toast(existingCategory ? "Category updated." : "Category created.");
      if (state.view === "sublist" && state.sublistContext?.kind === "category") {
        if (existingCategory && state.sublistContext.catId === existingCategory.id) renderCategoryDetail(existingCategory.id);
        else renderCategoriesSublist();
      } else {
        renderHome();
      }
    };
  ...
  async function deleteCategoryWithItems(cat) {
    const count = state.items.filter((i) => i.categoryId === cat.id).length;
    confirmDialog(
      count ? `Delete “${cat.name}”? The category will be removed, but its ${count} ${count === 1 ? "item" : "items"} will stay in MyVault without a category.` : `Delete “${cat.name}”?`,
      async () => {
        const affected = state.items.filter((i) => i.categoryId === cat.id);
        for (const item of affected) {
          item.categoryId = "";
          item.updatedAt = Utils.now();
          await MyVaultDB.putItem(item);
        }
        await MyVaultDB.deleteCategory(cat.id);
        state.activeCategoryId = null;
        state.sublistContext = null;
        await refreshData();
  ...
  function categoryOptions(selectedId) {
    return `<option value="" ${!selectedId ? "selected" : ""}>No category</option>` +
      state.categories.map((c) => `<option value="${c.id}" ${c.id === selectedId ? "selected" : ""}>${Utils.escapeHtml(c.name)}</option>`).join("");
  }
  ...
  function openItemForm(type, existingItem) {
    ...
    state.formImageBlobs = existingItem && Array.isArray(existingItem.imageBlobs) ? [...existingItem.imageBlobs] :
      (existingItem && existingItem.type === "image" && existingItem.fileBlob instanceof Blob ? [existingItem.fileBlob] : []);
    ...
    if (fileInput) {
      fileInput.addEventListener("change", async (e) => {
        ...
        if (effType === "image") {
          state.formImageBlobs = files;
          state.formFileBlob = files[0];
          renderFilePreview(form, effType, files);
        } else {
          const file = files[0];
  ...
  function renderFilePreview(form, type, file) {
    ...
    if (type === "image") {
      ...
      const previews = files.map((f) => {
        ...
        state.blobUrls.push(url);
        return `<div class="image-preview-tile"><img src="${url}" alt="" /><span>${Utils.escapeHtml(f.name)}</span></div>`;
      }).join("");
      wrap.innerHTML = `<div class="image-preview-grid">${previews}</div><div class="field-hint">${files.length} image${files.length === 1 ? "" : "s"} selected. They will be saved together as one Image item.</div>`;
      return;
    }
  ...
  function buildFormHtml(type, item, isQuick) {
    const defaultCategoryId = item ? item.categoryId : (state.activeCategoryId || myAppsCategoryDefaultFor(type));
    const categoryLocked = !!state.activeCategoryId && !isQuick && !item;
    const catField = categoryLocked ? `<div class="category-locked-note">Saving inside <strong>${Utils.escapeHtml(state.categories.find((c) => c.id === state.activeCategoryId)?.name || "this category")}</strong></div>` : `
      <label class="field-label">Category</label>
      <select class="field-select" id="form-category">${categoryOptions(defaultCategoryId)}</select>`;
    const tagsField = `
    ...
    const fileField = (accept, label) => `
      ...
      <input type="file" id="form-file-input" accept="${accept}" style="display:none;" ${type === "image" ? "multiple" : ""} ${((type === "image" && item && Array.isArray(item.imageBlobs) && item.imageBlobs.length) || (type !== "image" && item)) ? "" : "required"} />
    ...
    if (type === "note") {
      ...
      fields = `
        ${titleField("Reminder title")}
        ${descField("Note (optional)")}
        <label class="field-label">Remind me</label>
        <input class="field-input" type="datetime-local" id="form-reminder" required />
        ${catField}`;
    } else if (type === "task") {
      fields = `
        ...
        <input class="field-input" type="date" id="form-duedate" value="${item && item.dueDate ? toDateInput(item.dueDate) : toDateInput(Date.now())}" />
      ...
      const imageHint = type === "image" ? `<div class="field-hint">Choose multiple screenshots/images. They will stay together as one saved item.</div>` : "";
      fields = `
        ${fileField(accept, type === "image" ? "Images" : type === "video" ? "Video" : type === "document" ? "Document" : "File")}
        ${imageHint}
        ${titleField()}
        ${descField()}
        ${catField}${tagsField}${flagsField}`;
    ...
    function myAppsCategoryDefaultFor(t) {
      ...
      return state.activeCategoryId || (state.categories[0] ? state.categories[0].id : undefined);
    }
  }
  ...
  async function handleFormSubmit(e, type, existingItem, tagCtl, isQuick) {
    ...
    if ((type === "video" || type === "file" || type === "document") && !state.formFileBlob) {
      Utils.toast("Please choose a file first.", "error");
      return;
    }
    if (type === "image" && !state.formImageBlobs.length) {
      Utils.toast("Please choose at least one image.", "error");
      return;
    }
    ...
    if (type === "note") {
      ...
      if (!isQuick) {
        item.categoryId = state.activeCategoryId || get("#form-category").value;
        ...
        item.categoryId = state.activeCategoryId || undefined;
        item.tags = ["quick-capture"];
      }
    } else if (type === "reminder") {
      const rem = get("#form-reminder").value;
      if (!rem) { Utils.toast("Choose a reminder time.", "error"); return; }
      item.title = get("#form-title").value.trim();
      item.description = get("#form-description").value.trim();
      item.reminderAt = new Date(rem).getTime();
      item.categoryId = state.activeCategoryId || undefined;
      ...
      item.important = get("#form-important").checked;
      if (type === "image") {
        const blobs = Array.isArray(state.formImageBlobs) ? state.formImageBlobs.filter((b) => b instanceof Blob) : [];
        if (!blobs.length) { Utils.toast("Please choose at least one image.", "error"); return; }
        item.imageBlobs = blobs;
        item.imageNames = blobs.map((b) => b.name || "image");
        item.imageMimes = blobs.map((b) => b.type || "image/*");
        item.imageSizes = blobs.map((b) => b.size || 0);
        delete item.fileBlob;
        delete item.fileName;
        delete item.fileMime;
        delete item.fileSize;
      } else if (state.formFileBlob) {
  ...
  function rerenderCurrentView() {
    ...
    else if (state.view === "sublist" && state.sublistContext?.kind === "category") renderCategoryDetail(state.sublistContext.catId);
  }
  ...
  function openItemDetail(id) {
    ...
    if (item.type === "image") {
      ...
      if (urls.length) {
        media = `<div class="detail-media image-gallery">${urls.map((url, idx) => `<img src="${url}" alt="${Utils.escapeHtml((item.imageNames || [])[idx] || `Image ${idx + 1}`)}" loading="eager" />`).join("")}</div>`;
      }
    } else if (item.fileBlob instanceof Blob && item.type === "video") {
      media = `<div class="detail-media"><video src="${item._blobUrl}" controls></video></div>`;
    }
