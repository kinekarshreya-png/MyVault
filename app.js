...
(() => {
  ...
  function renderHome() {
    ...
    const todayTasks = state.items
      .filter((i) => i.type === "task" && !i.completed && i.dueDate && i.dueDate <= endOfToday.getTime())
    ...
    container.innerHTML = `
      <section class="notebook-section today-paper">
        <div class="notebook-section-head">
          <div><div class="paper-kicker">TODAY</div><h2>Things to do</h2></div>
  ...
  function buildFormHtml(type, item, isQuick) {
    ...
    const fileField = (accept, label) => `
      ...
      <input type="file" id="form-file-input" accept="${accept}" style="display:none;" ${type === "image" ? "multiple" : ""} ${item && type !== "image" ? "" : "required"} />
  ...
  function openItemDetail(id) {
    ...
    if (item.type === "image") {
      ...
      if (urls.length) {
        media = `<div class="detail-media image-gallery">${urls.map((url, idx) => `<img src="${url}" alt="${Utils.escapeHtml((item.imageNames || [])[idx] || `Image ${idx + 1}`)}" loading="lazy" />`).join("")}</div>`;
      }
    } else if (item.fileBlob instanceof Blob && item.type === "video") {
