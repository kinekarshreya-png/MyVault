...
const UI = (() => {
  ...
  function itemCard(item, categories) {
    ...
    if (item.type === "image") {
      ...
      if (urls.length) {
        thumb = `<div class="item-thumb-wrap"><img class="item-thumb" src="${urls[0]}" alt="" />${urls.length > 1 ? `<span class="image-count-badge">${urls.length} images</span>` : ""}</div>`;
      }
    }
    const flags = [];
  ...
  function taskRow(item, options = {}) {
    const dueBits = [];
    const d = item.dueDate ? Utils.daysUntil(item.dueDate) : null;
    if (item.dueDate) {
    ...
    const overdueActions = options.overdueActions && !item.completed && d !== null && d < 0 ? `
      <div class="overdue-task-actions">
        <span class="overdue-tag">${d === -1 ? "Yesterday" : `${Math.abs(d)} days overdue`}</span>
        <button type="button" class="overdue-action overdue-cancel" data-overdue-action="cancel" data-id="${item.id}">Cancel</button>
        <button type="button" class="overdue-action overdue-today" data-overdue-action="today" data-id="${item.id}">Move to today</button>
      </div>` : "";
    return `
