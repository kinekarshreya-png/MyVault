...
const UI = (() => {
  const TYPE_ICON = {
    ...
    video: "🎬", file: "📎", document: "📄", myapp: "🚀",
  };
  const TYPE_LABEL = {
    note: "Note", task: "Task", link: "Link", image: "Image",
    video: "Video", file: "File", document: "Document", myapp: "App",
  };
  ...
  function itemCard(item, categories) {
    ...
    let thumb = "";
    if ((item.type === "image") && item.fileBlob) {
      thumb = `<img class="item-thumb" src="${item._blobUrl || ""}" alt="" />`;
    }
    const flags = [];
    if (item.important) flags.push('<span class="flag-icon">💠</span>');
    if (item.favorite) flags.push('<span class="flag-icon">⭐</span>');
