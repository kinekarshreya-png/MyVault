  ...
  async function exportVault() {
    ...
    for (const item of items) {
      ...
      if (Array.isArray(copy.imageBlobs)) {
        ...
        copy.imageMimes = copy.imageBlobs.map((b) => b instanceof Blob ? b.type : "image/*");
        copy.imageSizes = copy.imageBlobs.map((b) => b instanceof Blob ? b.size : 0);
    ...
    for (const item of payload.items) {
      ...
      if (Array.isArray(copy.imageDataURLs)) {
        ...
        for (const dataURL of copy.imageDataURLs) {
          try {
            copy.imageBlobs.push(await dataURLToBlob(dataURL));
          } catch (e) {
